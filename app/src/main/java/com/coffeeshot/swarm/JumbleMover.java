 ...
 */
public class JumbleMover extends Mover
{
...
//    }
    ...
    if (mRenderables != null)
    {
      ...
      final boolean jumble = (time - mLastJumbleTime > JUMBLE_EVERYTHING_DELAY);
      if (jumble)
      {
        mLastJumbleTime = time;
      }

      for (int x = 0; x < mRenderables.size(); x++)
      {
        Renderable object = mRenderables.get(x);
        ...
        // Bounce.
        if ((object.x < mLeft && object.velocityX < 0.0f) || (object.x > mRight - object.width && object.velocityX > 0.0f))
        {
          object.velocityX = -object.velocityX * COEFFICIENT_OF_RESTITUTION;
          object.x = Math.max(0.0f, Math.min(object.x, mRight - object.width));
        ...
        if ((object.y < mTop && object.velocityY < 0.0f) || (object.y > mBottom - object.height && object.velocityY > 0.0f))
        {
          object.velocityY = -object.velocityY * COEFFICIENT_OF_RESTITUTION;
          object.y = Math.max(0.0f, Math.min(object.y, mBottom - object.height));
